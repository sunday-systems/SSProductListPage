 ...
 * @version $Id$
 */
class SS_Page_Admin_Design_ProductList_Ex extends LC_Page_Admin_Ex
{
    /**
     * Page を初期化する.
     *
    ...
    {
        parent::init();
        $this->tpl_mainpage = PLUGIN_SS_PRODUCTLIST_TPL . 'design/ss_product_list.tpl';
        $this->tpl_subno = 'layout';
        $this->tpl_mainno = 'design';
        $this->tpl_maintitle = 'デザイン管理';
        $this->tpl_subtitle = '商品リストレイアウト設定';
    ...
    {
        $objLayout = new SS_Helper_ProcucListPageLayout_Ex();
        ...
        switch ($this->getMode()) {
            ...
            case 'preview':
                ...
                //SC_Response_Ex::sendRedirectFromUrlPath('preview/' . DIR_INDEX_PATH, array('filename' => $filename, 'ss_product_list_page_id' => $this->page_id));
                SC_Response_Ex::sendRedirectFromUrlPath('products/list.php', array('category_id' => $this->page_id, 'preview' => 1));
                SC_Response_Ex::actionExit();
     ...
     * @param  SS_Helper_PageLayout $objLayout      SC_Helper_PageLayout インスタンス
    ...
    public function savePreviewData($page_id, &$objLayout)
    {
        ...
        if (!SC_Query_Ex::getSingletonInstance()->exists('plg_product_list_pagelayout', 'page_id = 0')) {
            SC_Query_Ex::getSingletonInstance()->insert('plg_product_list_pagelayout', array(
                'page_id' => $page_id,
            ...
            $objQuery->update('plg_product_list_pagelayout', $arrPageData[0], 'page_id = 0 AND device_type_id = ?', array(DEVICE_TYPE_PC));
        }
    ...
    public function placingBlocs(&$objFormParam, $is_preview = false)
    {
        ...
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $objQuery->begin();
        if (!SC_Query_Ex::getSingletonInstance()->exists('plg_product_list_pagelayout', 'page_id = ? AND device_type_id = ?', array($page_id, $device_type_id))) {
            SC_Query_Ex::getSingletonInstance()->insert('plg_product_list_pagelayout', array(
                'page_id' => $page_id,
        ...
        $objQuery->delete('plg_product_list_blocposition', 'page_id = ? AND device_type_id = ?',
        ...
        for ($i = 1; $i <= $bloc_cnt; $i++) {
            ...
            if ($arrParams['anywhere'] == 1) {
                $exists = $objQuery->exists('plg_product_list_blocposition', 'anywhere = 1 AND bloc_id = ? AND device_type_id = ?',
            ...
            $objQuery->insert('plg_product_list_blocposition', $arrParams);
        }
        $objQuery->commit();
    }
}
